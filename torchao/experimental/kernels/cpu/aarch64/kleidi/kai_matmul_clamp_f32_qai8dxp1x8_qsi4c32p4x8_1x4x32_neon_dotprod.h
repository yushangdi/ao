...
void prepare_weight_data(
  ...
  for (size_t i = 0; i < n * k; i += 2) {
    uint8_t high = reinterpret_cast<const uint8_t*>(weight_qvals)[i] + wzp;
    uint8_t low = reinterpret_cast<const uint8_t*>(weight_qvals)[i + 1] + wzp;
    packed_weight_qvals[i / 2] = ((high << 4) | (low & 0xF));
  }

  // Parameters for packing
  rhs_packing::qparams_t qparams{
      /*lhs_zp=*/1, wzp, .scale_dt = kai_datatype::kai_dt_bf16};
  ...
  rhs_pack.run_rhs_pack(
      ...
      /*rhs_stride=*/(k + 1) / 2, // = roundup(k, 2) / 2
      /*bias=*/nullptr,
      /*scale=*/reinterpret_cast<const uint16_t*>(weight_scales_bf16.data()),
      /*scale_stride=*/sizeof(uint16_t) * (k + group_size - 1) /
          group_size, // = roundup(k, group_size) / group_size
